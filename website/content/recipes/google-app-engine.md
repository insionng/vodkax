...
description = "Google App Engine recipe / example for Vodka"
[menu.side]
...
A separate source file contains the function to create the Vodka instance and add the static
...
The handler-wireup that would normally also be a part of this Vodka setup moves to separate files which
take advantage of the ability to have multiple `init()` functions which run _after_ the `e` Vodka var is
...
Vodka instance directly (so things like CORS middleware can be added at this higher common-level).
...
So far we've seen how to split apart the Vodka creation and setup but still have the same app that
...
This allows us to replace the `createMux()` function to create our Vodka server _without_ any of the
