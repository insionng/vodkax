...
func TestCSRFTokenFromForm(t *testing.T) {
	...
	f.Set("csrf", "token")
	e := vodka.New()
	req := test.NewRequest(vodka.POST, "/", strings.NewReader(f.Encode()))
	req.Header().Add(vodka.HeaderContentType, vodka.MIMEApplicationForm)
...
func TestCSRFTokenFromQuery(t *testing.T) {
	...
	q.Set("csrf", "token")
	e := vodka.New()
	req := test.NewRequest(vodka.GET, "/?"+q.Encode(), nil)
	req.Header().Add(vodka.HeaderContentType, vodka.MIMEApplicationForm)
	c := e.NewContext(req, nil)
