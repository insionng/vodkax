...
import (
	...
	"github.com/insionng/gommon/bytes"
)
...
// G, T or P.
func BodyLimit(limit string) vodka.MiddlewareFunc {
...
// See: `BodyLimit()`.
func BodyLimitWithConfig(config BodyLimitConfig) vodka.MiddlewareFunc {
	// Defaults
	...
	return func(next vodka.HandlerFunc) vodka.HandlerFunc {
		return func(c vodka.Context) error {
			...
			if req.ContentLength() > config.limit {
				return vodka.ErrStatusRequestEntityTooLarge
			}
...
func (r *limitedReader) Read(b []byte) (n int, err error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, vodka.ErrStatusRequestEntityTooLarge
	}
	return
}

func (r *limitedReader) Reset(reader io.Reader, context vodka.Context) {
	r.reader = reader
	r.context = context
}
