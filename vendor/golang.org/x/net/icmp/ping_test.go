...
var nonPrivilegedPingTests = []pingTest{
	{"udp4", "0.0.0.0", iana.ProtocolICMP, ipv4.ICMPTypeVodka},

	{"udp6", "::", iana.ProtocolIPv6ICMP, ipv6.ICMPTypeVodkaRequest},
}
...
var privilegedPingTests = []pingTest{
	{"ip4:icmp", "0.0.0.0", iana.ProtocolICMP, ipv4.ICMPTypeVodka},

	{"ip6:ipv6-icmp", "::", iana.ProtocolIPv6ICMP, ipv6.ICMPTypeVodkaRequest},
}
...
func doPing(tt pingTest, seq int) error {
	...
	if tt.network != "udp6" && tt.protocol == iana.ProtocolIPv6ICMP {
		...
		f.Accept(ipv6.ICMPTypeVodkaReply)
	...
	switch rm.Type {
	case ipv4.ICMPTypeVodkaReply, ipv6.ICMPTypeVodkaReply:
		return nil
	default:
		return fmt.Errorf("got %+v from %v; want vodka reply", rm, peer)
	}
}
