...
func startServer() {
	http.Handle("/vodka", Handler(vodkaServer))
...
func TestVodka(t *testing.T) {
	...
	conn, err := NewClient(newConfig(t, "/vodka"), client)
	if err != nil {
	...
	if !bytes.Equal(msg, actual_msg) {
		t.Errorf("Vodka: expected %q got %q", msg, actual_msg)
	}
	conn.Close()
}
...
func TestAddr(t *testing.T) {
	...
	if !strings.HasPrefix(ra, "ws://") || !strings.HasSuffix(ra, "/vodka") {
...
func TestWithQuery(t *testing.T) {
	...
	config := newConfig(t, "/vodka")
	config.Location, err = url.ParseRequestURI(fmt.Sprintf("ws://%s/vodka?q=v", serverAddr))
	if err != nil {
...
func TestHTTP(t *testing.T) {
	...
	resp, err := http.Get(fmt.Sprintf("http://%s/vodka", serverAddr))
	if err != nil {
...
func TestSmallBuffer(t *testing.T) {
	...
	if !bytes.Equal(msg[:len(small_msg)], small_msg) {
		t.Errorf("Vodka: expected %q got %q", msg[:len(small_msg)], small_msg)
	}
	...
	if !bytes.Equal(msg[len(small_msg):], second_msg) {
		t.Errorf("Vodka: expected %q got %q", msg[len(small_msg):], second_msg)
	}
	conn.Close()
}
...
func TestClose(t *testing.T) {
	...
	client, err := NewClient(newConfig(t, "/vodka"), &cc)
	if err != nil {
